...
int insertAtFront(ListT *listPtr, NodeT *p) //function to insert a note at the front of the list
{
...
NodeT *createNode(int key)//function to create a node - here used when we insert in the list
{
...
int main(int argc, char* argv[])
{
    ...
    //count the number of commands (=lines) given in the file:
    ...
    for(int i=0; i<lines; i++)
    {
        ...
        case 'E':
            ...
            {
                //check if the car can get out of the road ie it must be the first who entered the road:

                if(road->first->key!=value) fprintf(pf,"error: %d not at exit!\n",value);
                else
                {
        ...
        default:
            fprintf(pf,"The command %c is not valid",command[0]);
